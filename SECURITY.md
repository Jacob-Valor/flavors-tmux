...
and environment variables, executes shell commands and a Zig binary for widget
...
| Theme colors are hardcoded in built-in themes or validated as `#[0-9A-Fa-f]{6}` in custom JSON themes | `scripts/themes.sh`, `src/core/theme_loader.zig` |
| `#` in user-supplied icon values is escaped to `##` to prevent tmux format injection | `flavors.tmux` |
| Forge tokens are read from environment variables only — tmux option storage is intentionally unsupported (CWE-522) | `src/widgets/wb_git_status.zig`, `scripts/wb-git-status.sh` |
| Token never appears in process command-line argv — written to a temp file and loaded via `curl -K` (CWE-214) | `src/widgets/wb_git_status.zig` |
| Auto-update verifies the remote URL matches `Jacob-Valor/flavors-tmux` before pulling (CWE-494) | `scripts/auto-update.sh` |
| Custom theme names are restricted to `^[A-Za-z0-9_-]+$` to prevent path traversal | `src/core/theme_loader.zig`, `scripts/themes.sh` |
